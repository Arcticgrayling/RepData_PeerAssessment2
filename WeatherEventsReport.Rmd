...
```
Next I used an update table to make changes to the remaining incorrect event types.  I had create the update table by hand, but I am including a print out of it below so it can be reproduced.
...
g <- ggplot(d1, aes(EventType, EconCost))
...
#### Session information for the system used to create this report:
```{r}
sessionInfo()
```