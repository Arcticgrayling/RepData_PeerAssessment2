...
```{r}
#data$EconCost <- data %>% sapply(data$value,function(x)sum(x < data$value) / nrow(data))
data <- mutate(data, EconCost = mapvalues(CropDamage, NA, 0) + 
                       mapvalues(PropertyDamage, NA, 0))
#data <- mutate(data, EconCost = cumsum(CropDamage, PropertyDamage, na.rm=TRUE))
```
...
System info for setup used to create this report:
```{r}
sessionInfo()
```